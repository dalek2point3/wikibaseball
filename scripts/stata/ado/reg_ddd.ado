...
esttab using "${tables}`ln'ddd.tex", keep(`int' `x1' `x2') se ar2 nonotes star(+ 0.15 * 0.10 ** 0.05 *** 0.01) coeflabels(`int' "Out-of-Copy X Post X Baseball" `x1' "Out-of-Copy X Post" `x2' "Baseball X Post") mtitles("Images" "Text" "Citations" "Traffic") replace booktabs s(fixed sstt r2_a N N_g, label("Player FE" "Group X Year FEs" "adj. \$R^2\$" N "Clusters")) width(\hsize)
